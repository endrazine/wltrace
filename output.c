...
#define DEFAULT_BLACKLIST "/etc/ltrace.bl"

int witchcraft_ret(uint64_t v, unsigned int retnum);
...
static unsigned long int ret_ctn = 1;	// start from this number
static unsigned long int ctx_ctn = 1;
...
//		current_column += fprintf(options.output, "[pid %u] ", proc->pid);
	...
	if(options.witchcraft == WITCHRAFT_C) {
		fprintf(options.output, "    void *ret_%lu = ", ret_ctn++);
	}else if(options.witchcraft == 0){
		;
	}else if(options.witchcraft == 1){
		fprintf(options.output, "    ret_%lu = ", ret_ctn++);
	}else{
		fprintf(options.output, "    ret_%lu, ctx_%lu = ", ret_ctn++, ctx_ctn++);
	}
...
void rd_blacklist() {
	...
	fp = fopen(DEFAULT_BLACKLIST, "r");
	if (fp == NULL){ printf(" !! Error open() : %s\n", strerror(errno)); return; }

	while ((read = getline(&line, &len, fp)) != -1) {
//        printf("Retrieved line of length %zu :\n", read);
//		printf("%s", line);
		blacklist[blacklist_cnt] = strdup(line);	
		blacklist_cnt++;
	}
...
// If function is within blacklist, start line with a comment
void
comment_out_blacklist(const char *function_name)
{
	unsigned int i = 0;
	...
	for(i = 0; i < blacklist_cnt; i++){
		if(!strncmp(blacklist[i], function_name, strlen(function_name))){
			if(options.witchcraft == WITCHRAFT_C){
				fprintf(options.output,"// ");
			}else if(!options.witchcraft){
				;
			}else{
				fprintf(options.output,"-- ");
			}
	    ...
	    struct library_symbol *libsym)
{
	...
	Function *func;
//
// If we are under exit(), stop tracing
//
	...
	if (options.bt_depth > 0) {
		...
		while (unwind_depth) {
			...
			if(!strncmp("exit", fn_name, 4)){ within_exit = 1; break; };
...
//		fprintf(options.output, "\n");
	...
	comment_out_blacklist(function_name);
...
*/
	...
	if(options.witchcraft){

		switch(options.witchcraft) {
			case 1:
				fprintf(options.output, "; -- ");
				break;
			case 2:
				fprintf(options.output, "; print(\"ret_%lu = \", ret_%lu); -- (%p) ", ret_ctn - 1, ret_ctn - 1, (void*)&retval.u);
				break;
			case 3:
				;
				char *cmd = calloc(4096,1);
				snprintf(cmd, 4095, "; fprintf(2, \"ret_%lu = %%p\\n\", ret_%lu);", ret_ctn - 1, ret_ctn - 1);
				fprintf(options.output, "%s", cmd);
				break;

			case WITCHRAFT_C:
				fprintf(options.output, "; // (%p) ", (void*)&retval.u);
			default:
				break;
		}
	...
	if(options.witchcraft){
		witchcraft_ret(*(uint64_t*)&retval.u, ret_ctn -1);
	}
