...
struct options_t options = {
	...
	.witchcraft = 0,              /* output as executable witchcraft script */
};
...
# endif
		...
		"  -W, --witchcraft    Generate executable witchcraft script.\n"
...
# ifdef USE_DEMANGLE
		...
		c = getopt_long(argc, argv, "+cfhiLrStTVbW"
# ifdef USE_DEMANGLE
				"C"
# endif
#if defined(HAVE_LIBUNWIND)
				"a:A:D:e:F:l:n:o:p:s:u:x:X:w:W::", long_options,
		...
		case 'W':
			if(optarg){
				if(!strncmp(optarg, "C", 1)){
					options.witchcraft = WITCHRAFT_C;
				}else {
					options.witchcraft = atoi(optarg);
				}
			}else{
				options.witchcraft++;
			}
