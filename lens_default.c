...
int addrcmp(variable *a, variable *b) {
    return b->addr - a->addr;
}

variable *head = NULL;

int witchcraft_var(FILE *stream, uint64_t v){
	int ret = 0;
	variable * var = 0, evar;

	// search v in our list of return values
	evar.addr = v;

	DL_SEARCH(head, var, &evar, addrcmp);
	if(var){
		ret = fprintf(stream, "%s", var->name);
	}else{
		ret = fprintf(stream, PREFIX_VAR"%#"PRIx64, v);
	}
...
int witchcraft_ret(uint64_t v, unsigned int retnum){
	variable * var = 0, *nvar, evar;

	if(!v){ return 0; }

        if ( (nvar = (variable*)malloc(sizeof(variable))) == NULL) { printf(" !! Error: malloc() : %s\n", strerror(errno)); return -1; };
	nvar->addr = v;
	snprintf(nvar->name, BUFLEN-1, "ret_%u", retnum);
...
#define HANDLE_WIDTH(BITS)						\
	do {								\
		...
		case INT_FMT_unknown:					\
			if (l < -10000 || l > 10000)			\
		case INT_FMT_x:						\
			return witchcraft_var(stream, v); \
		case INT_FMT_i:						\
